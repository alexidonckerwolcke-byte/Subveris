...
import type { IStorage, InsertSubscriptionData } from "./storage.js";
...
function generateOpportunityCosts(monthlyAmount: number, currency: string = 'USD'): { item: string; count: number; icon: string }[] {
...
export class SupabaseStorage implements IStorage {
  ...
  async trackUsageByDomain(userId: string, domain: string, timeSpent: number): Promise<Subscription | undefined> {
    // Try to find a subscription owned by the user first.
    ...
    if ((!data || error) && userId) {
      // If no direct owner subscription exists, check family shared subscriptions.
      ...
      if (!memberGroupsError && Array.isArray(memberGroups)) {
        groupIds.push(...memberGroups.map((group: any) => group.family_group_id).filter(Boolean));
      }
      ...
      if (!ownedGroupsError && Array.isArray(ownedGroups)) {
        groupIds.push(...ownedGroups.map((group: any) => group.id).filter(Boolean));
      }

      const uniqueGroupIds = Array.from(new Set(groupIds));
      if (uniqueGroupIds.length > 0) {
        const { data: sharedSubscriptions, error: sharedSubscriptionsError } = await supabase
          .from('shared_subscriptions')
          .select('subscription_id')
          .in('family_group_id', uniqueGroupIds);

        if (!sharedSubscriptionsError && Array.isArray(sharedSubscriptions) && sharedSubscriptions.length > 0) {
          const subscriptionIds = Array.from(new Set(sharedSubscriptions
            .map((row: any) => row.subscription_id)
            .filter(Boolean)));

          if (subscriptionIds.length > 0) {
            const { data: sharedDomainSubscription, error: sharedDomainError } = await supabase
              .from('subscriptions')
              .select('*')
  ...
  async updateSubscriptionNextBilling(id: string, nextBillingDate: string): Promise<Subscription | undefined> {
    // Normalize date format to local date-only (YYYY-MM-DD) before storing.
    let normalized = nextBillingDate;
    try {
      const dateStr = String(nextBillingDate).split('T')[0];
  ...
  async getMetrics(): Promise<DashboardMetrics> {
    ...
    function getBillingMonth(sub: any): string | null {
    ...
    function parseRenewalDate(dateStr?: string | null): Date | null {
    ...
    const totalMonthlySpend = subscriptions
      ...
      .reduce((sum, sub) => {
        ...
        const billingMonth = getBillingMonth(sub);

        const renewalDate = parseRenewalDate(
          (sub as any).next_billing_at || (sub as any).nextBillingDate || (sub as any).next_billing_date || (sub as any).next_billing);

        let billedInMonth = false;
        if (billingMonth === targetMonth) {
          if (renewalDate) {
            if (renewalDate <= now) billedInMonth = true;
            else if (`${renewalDate.getFullYear()}-${String(renewalDate.getMonth() + 1).padStart(2, '0')}` !== targetMonth) billedInMonth = true;
          }
        } else {
          if (renewalDate) {
