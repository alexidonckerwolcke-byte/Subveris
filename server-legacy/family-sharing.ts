...
import type { FamilyGroup, FamilyGroupMember, SharedSubscription, CostSplit, CalendarEvent } from '../shared/schema.js';
...
export function generateAIRecommendations(subs: any[]): any[] {
  ...
  const getUsageValue = (sub: any): number | null => {
  ...
  const isStreamingService = (sub: any) => {
  ...
  const isProductivityService = (sub: any) => {
    const lower = normalizeText(sub.name);
  ...
  const isCloudStorageService = (sub: any) => {
    const lower = normalizeText(sub.name);
    return sub.category === "cloud-storage" || /(dropbox|google drive|icloud|onedrive|box|cloud storage|storage)/.test(lower);
  };

  const isAnnualPlan = (sub: any) => normalizeText(sub.frequency) === "yearly";

  const validSubs = subs.filter((sub: any) => sub && (sub.status === "unused" || sub.status === "to-cancel"));

  const activeStreamingSubs = validSubs.filter((sub: any) => sub.status === "active" && isStreamingService(sub));
  const streamingTotal = activeStreamingSubs.reduce((sum: number, sub: any) => sum + monthlyAmountFor(sub), 0);
  if (activeStreamingSubs.length >= 2 && streamingTotal >= 30) {
    const sub = activeStreamingSubs[0];
    recommendations.push({
      id: randomUUID(),
      type: "downgrade",
      title: "Consolidate or rotate your streaming services",
      description: `You are paying ${streamingTotal.toFixed(2)} ${sub.currency || "USD"}/mo for ${activeStreamingSubs.length} streaming services. Pausing one service and rotating selections can lower that spend by roughly one-third without losing access to favorites.`,      currentCost: streamingTotal,
  ...
  for (const sub of validSubs) {
    try {
      ...
      const id = randomUUID();
      const isStreaming = isStreamingService(sub);
      const isProductivity = isProductivityService(sub);
      const isCloudStorage = isCloudStorageService(sub);
      const isAnnual = isAnnualPlan(sub);
      ...
      if (sub.status === "unused") {
        ...
        recommendations.push({
          ...
          type: "cancel",
          title: `Cancel ${name}`,
          description: `${usageMessage} ${name} costs ${monthly.toFixed(2)} ${currency}/mo. Canceling now avoids another renewal.${renewalText}`,
          currentCost: monthly,
          suggestedCost: 0,
          savings: monthly,
          subscriptionId: sub.id,
      ...
      if (sub.status === "to-cancel") {
        recommendations.push({
          ...
          description: `${name} is already marked for cancellation. Finalize it before the next bill to stop future charges.${renewalText}`,
          currentCost: monthly,
          suggestedCost: 0,
          savings: monthly,
          subscriptionId: sub.id,
      ...
      if (sub.status === "active") {
        if (usage === 0 && monthly >= 8) {
          recommendations.push({
            ...
            description: `You did not use ${name} this month, yet it costs ${monthly.toFixed(2)} ${currency}/mo. Cancel it to reclaim budget immediately.${renewalText}`,
            currentCost: monthly,
            suggestedCost: 0,
            savings: monthly,
            subscriptionId: sub.id,
        ...
        if (isProductivity && monthly >= 12) {
          recommendations.push({
            ...
            description: `${name} costs ${monthly.toFixed(2)} ${currency}/mo. If your work is primarily documents, notes, or casual design, a simpler alternative could cut your spend by more than 40%.`,            currentCost: monthly,
        ...
        if (isStreaming && monthly >= 14 && (usage === null || usage <= 3)) {
          recommendations.push({
            ...
            description: `${name} costs ${monthly.toFixed(2)} ${currency}/mo. If you only watch it occasionally, a shared plan or alternating between services could lower your monthly streaming bill.`,            currentCost: monthly,
        ...
        if (isCloudStorage && monthly >= 10 && (usage === null || usage <= 2)) {
          recommendations.push({
            ...
            description: `${name} costs ${monthly.toFixed(2)} ${currency}/mo. If you are using little of the capacity, a lower tier can keep your files safe while trimming the bill.`,            currentCost: monthly,
            suggestedCost: Math.round((monthly * 0.65) * 100) / 100,
            savings: Math.round((monthly * 0.35) * 100) / 100,
            subscriptionId: sub.id,
            confidence: 0.84,
        ...
        if (isAnnual && monthly >= 15) {
          recommendations.push({
            ...
            description: `${name} is currently billed annually. Check whether switching to monthly, negotiating a loyalty discount, or using a promo can reduce the effective monthly cost.`,            currentCost: monthly,
        ...
        if (monthly >= 25) {
          ...
          recommendations.push({
            ...
            description: `${name} costs ${monthly.toFixed(2)} ${currency}/mo. ${usageDescriptor} A lower tier, promotion, or alternate vendor could shrink this cost by 20–40%.`,            currentCost: monthly,
        ...
        if (usage !== null && usage <= 2 && monthly >= 10) {
          recommendations.push({
            ...
            description: `You used ${name} only ${usage} time${usage === 1 ? "" : "s"} this month, while it costs ${monthly.toFixed(2)} ${currency}/mo. A smaller plan may still cover what you need.`,            currentCost: monthly,
...
export async function downgradeFromFamilyPlan(userId: string, familyGroupId: string): Promise<void> {
  ...
  let { data: backup, error: backupError } = await supabase
    .from('family_group_plan_backups')
  ...
  // If no backup found for this specific group, check for a backup with empty family_group_id
  // (this handles the case where the user was upgraded to family plan before joining a specific group)
  if (!backup) {
    const { data: generalBackup, error: generalBackupError } = await supabase
      .from('family_group_plan_backups')
      .select('original_plan_type, original_status')
      .eq('user_id', userId)
      .eq('family_group_id', '')
    ...
    if (generalBackupError && generalBackupError.code !== 'PGRST116') {
      throw new Error(`Failed to fetch general plan backup: ${generalBackupError.message}`);
    }
...
export async function createFamilyGroup(userId: string, name: string): Promise<FamilyGroup> {
  ...
  if (ownedGroupsError) {
    throw new Error(`Failed to check existing family groups: ${ownedGroupsError.message}`);
  }

  if ((ownedGroups?.length ?? 0) >= 1) {
  ...
  try {
    ...
    if (supabaseAdmin.auth && supabaseAdmin.auth.admin) {
      ...
      const userPayload = userData as any;
...
export async function addFamilyMember(groupId: string, userId: string, memberUserId: string): Promise<FamilyGroupMember> {
  ...
  if (existingMembersError) {
    throw new Error(`Failed to check family group size: ${existingMembersError.message}`);
  }

  if ((existingMembers?.length ?? 0) >= 5) {
...
export async function getFamilyMembers(groupId: string): Promise<FamilyGroupMember[]> {
  ...
  const missingEmailIds = Array.from(
    new Set(members.filter((m: any) => !m.email && m.user_id).map((m: any) => m.user_id))
  );
  ...
  const result = members.map((m: any) => ({
    ...
    email: m.email ?? emailById[m.user_id] ?? null,
  }));

  await Promise.all(
    members
      .filter((m: any) => !m.email && emailById[m.user_id])
      .map((m: any) =>
        supabase
          .from('family_group_members')
