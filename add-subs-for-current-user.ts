...
async function main() {
  ...
  // Find user by email alexi.donckerwolcke@gmail.com
  let user = users?.find(u => u.email === 'alexi.donckerwolcke@gmail.com');
  
  if (!user) {
    console.log('User not found with that email, using first user');
    user = users?.[0];
  }
  ...
  console.log('Adding subscriptions for:', userId, email);
  ...
  console.log('Done! Subscriptions created for:', email);
}
